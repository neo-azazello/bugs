...
class TaskChecklistController extends Controller
{
    ...
    public function markTaskChecklist($request, $response)
    {
        ...
        $is_done = $this->container->db
            ->table('taskchecklist')
            ->select('is_done')->where('id', $request->getParam('id'))
            ->value('is_done');

        if ($is_done == '1') {
            $task = $this->container->db
                ->table('taskchecklist')
    ...
    public function addNewTaskChecklistComment($request, $response)
    {
        ...
        $new_comment = $this->container->db
            ->table('taskchecklistcomments')
            ->insert(array(
                'taskid' => $taskid,
        ...
        $this->container->telegram->addNewTaskChecklistComment($taskid, $checklistid);
    ...
    public function editChecklistComment($request, $response)
    {
        ...
        $update_comment = $this->container->db
            ->table('taskchecklistcomments')
